...
var list = async function(memno){
    ...
    await sql('SELECT product.*, product.memno AS promemno, product.picture AS propicture, member.*, ordmaster.*, orddetails.amt, payment.payname FROM ordmaster JOIN orddetails ON ordmaster.ordno = orddetails.ordno JOIN product ON orddetails.prono = product.prono JOIN member ON product.memno=member.memno JOIN payment ON payment.payno = ordmaster.payno WHERE ordmaster.memno=$1 ORDER BY ordno', [memno])
        .then((data) => {
    ...
    await sql('SELECT product.*, product.memno AS promemno, product.picture AS propicture, member.*, ordmaster.*, orddetails.amt, payment.payname FROM ordmaster JOIN orddetails ON ordmaster.ordno = orddetails.ordno JOIN product ON orddetails.prono = product.prono JOIN member ON ordmaster.memno=member.memno JOIN payment ON payment.payno = ordmaster.payno WHERE product.memno=$1 AND ordmaster.stat<>\'D\' AND ordmaster.stat<>\'S\' ORDER BY ordno', [memno])
        .then((data) => {            
